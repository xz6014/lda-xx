...
function (documents, K, vocab, num.e.iterations, num.m.iterations,
    ...
    lambda = 10, regularise = FALSE, method = "sLDA", trace = 0L, MaxNWts=3000, initial = NULL, compute.log.likelihood = FALSE, freeze.topics=FALSE)
{
    ...
    result <- .slda.collapsed.gibbs.sampler(documents, K, vocab,
        ...
        variance[1], logistic, method[1], lambda = lambda, trace = trace, initial = initial, compute.log.likelihood = compute.log.likelihood,freeze.topics=freeze.topics)
    ...
    for (ii in seq(length.out = num.m.iterations - 1)) {
        result <- .slda.collapsed.gibbs.sampler(documents, K,
            ...
            lambda = lambda, initial = list(assignments = result$assignments), compute.log.likelihood = compute.log.likelihood,
            trace = trace,freeze.topics=freeze.topics)
        fit <- estimate.params(result$document_sums, K, logistic,MaxNWts)
        params <- fit$coefs
    }
    c(result, fit)
